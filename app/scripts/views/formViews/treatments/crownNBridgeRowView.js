define([
	...
	var CrownNBridgeRowView = Backbone.View.extend({
		...
		className: 'crownNBridgeRow row-fluid',
		events: {
		...
		exitColumn: function(ev) {
			...
			switch(targetClass){
				...
				case "tooth":
					...
					break;
				case "numCrownNBridge":
					this.model.set(targetClass,parseInt(this.$('.numCrownNBridge').attr('value'),10));
					this.model.isValid(true);
					break;
			}
		...
		addNewExpendableInventoryItem: function(value){
			...
			vent.trigger('CDF.Views.Treatments.CrownNBridgeRowView:addNewExpendableInventoryItem',{genericName:value,callback:newExpendableInventoryItemAdded});
		},
		...
		addNewTreatment: function(propertyName){
			   var self = this;
				function newTreatmentAdded(treatmentModel) {
     			...
     			vent.trigger('CDF.Views.Treatments.CrownNBridgeRowView:addNewTreatment',{name:propertyName,category:1005,callback:newTreatmentAdded});
		},
		...
		render: function() {
			...
			this.$('.treatment').typeahead({source:treatmentsSource(new Treatments(),1005),updater:treatmentsUpdater,minLength:3,id:"treatment"+this.model.cid,map:this.treatmentsMap});
			...
			this.$('.stage').typeahead({source:stagesSource(1005),updater:stagesUpdater,minLength:1,id:"stage"+this.model.cid,map:this.stagesMap});

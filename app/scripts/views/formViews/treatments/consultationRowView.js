define([
	...
	'vent',
	'text!templates/consultationRow.html',
	'text!templates/yesNo.html',
	'config',
	'bootstrap',
	
	], function(Backbone,$,_,utility,Persons,Treatments,ConsultationRow,vent,template,yesNoTemplate,config){

	var ConsultationRowView = Backbone.View.extend({
		//model: new ConsultationRow(),
		className: 'ConsultationRow row-fluid',
		events: {
		...
		addNewPatient: function(propertyName){
				...
				vent.trigger('CDF.Views.Treatments.ConsultationRowView:addNewPatient',{patientNameString:propertyName,callback:newPatientAdded});
		},
		addNewDoctor: function(propertyName){
			   var self = this;
				...
				vent.trigger('CDF.Views.Treatments.ConsultationRowView:addNewDoctor',{doctorNameString:propertyName,callback:newDoctorAdded});
		...
		addNewTreatment: function(propertyName){
			   var self = this;
				function newTreatmentAdded(treatmentModel) {
     			...
     			vent.trigger('CDF.Views.Treatments.ConsultationRowView:addNewTreatment',{name:propertyName,category:1000,callback:newTreatmentAdded});
		},
		...
		delete: function(ev) {
			...
			vent.trigger('CDF.Views.Treatments.ConsultationRowView:delete');			
			this.close();
		},
		onValid: function(view,errors){
			var self = this;
			vent.trigger('CDF.Views.Treatments.ConsultationRowView:onValid');
		...
		render: function() {
			...
			this.$('.treatment').typeahead({source:treatmentsSource(new Treatments(),1000),updater:treatmentsUpdater,minLength:3,id:"treatment"+this.model.cid,map:this.treatmentsMap});
